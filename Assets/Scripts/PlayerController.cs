...
public class PlayerController : MonoBehaviour {
    ...
    private float attack_comeback_timer = 0f;
    ...
    void FixedUpdate() {
        ...
        if (!is_attacking() && input_attack && attack_comeback_timer == 0) {
            attack_comeback_timer = 50;
            if (is_running) {
        ...
        if(attack_comeback_timer > 0){
            attack_comeback_timer --;
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D other){
        if (other.tag == "checkpoint") {
            ...
            respawnpoint = CheckpointsArr[outerRnd.Next(0, 12)].transform.position;
